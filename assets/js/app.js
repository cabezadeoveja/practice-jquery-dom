$(document).ready(() => {
  ...
  * evento de click en el botón que queremos y le decimos que una vez ejecutado este evento
  * recorra el largo del arreglo donde están almacenados los datos de los personajes de
    ...
    for (var i = 0; i < gossip.length; i++) {
      ...
      */
      $('.gossip').append('<div class="row character">' +
        '<div class= "col-md-6 text-center">' + '<img src="assets/' + gossip[i].picture + '">' + '</div>' + '<div class= "col-md-6">' + '<h3>' + gossip[i].name + '</h3>' + '<p>School: ' + gossip[i].school + '</p>' + '<p>City: ' + gossip[i].city + '</p>' + '</div>' + '</div>');
    }
  });
  ...
  */
  ...
  $('.btn-breaking').click(function() {
    for (var i = 0; i < breaking.length; i++) {
      ...
      $('.breaking').append('<div class="row character">' + '<div class= "col-md-6 text-center">' + '<img src="assets/' + breaking[i].picture + '">' + '</div>' + '<div class= "col-md-6">' + '<h3>' + breaking[i].name + '</h3>' + '<p>City: ' + breaking[i].city + '</p>' + '</div>' + '</div>');
    }
  });
  ...
  $('.btn-how').click(function() {
    for (var i = 0; i < howtoget.length; i++) {
      ...
      $('.how').append('<div class="row character">' +
        ...
        '</div>' +
        '<div class= "col-md-6">' + '<h3>' + howtoget[i].name + '</h3>' + '<p>City: ' + howtoget[i].city + '</p>' + '</div>' + '</div>');
    }
  });
  ...
  $('.btn-stranger').click(function() {
    for (var i = 0; i < stranger.length; i++) {
      ...
      $('.stranger').append('<div class="row character">' + '<div class= "col-md-6 text-center">' + '<img src="assets/' + stranger[i].picture + '">' + '</div>' + '<div class= "col-md-6">' + '<h3>' + stranger[i].name + '</h3>' + '<p>City: ' + stranger[i].city + '</p>' + '</div>' + '</div>');
    }
  });
    ...
    */
    for (var i = 0; i < allCharacters.length; i++) {
      for (var j = 0; j < allCharacters.length; j++) {
        $('.all').append('<div class="row character">' + '<div class= "col-md-6 text-center">' + '<img src="assets/' + allCharacters[i][j].picture + '">' + '</div>' + '<div class= "col-md-6">' + '<h3>' + allCharacters[i][j].name + '</h3>' + '<p>City: ' + allCharacters[i][j].city + '</p>' + '</div>' + '</div>');
      }
    }
  });
});
